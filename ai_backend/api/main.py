...
from ai_backend.storage.video import save_video
from ai_backend.inference.fall_detection_pipeline import process_frame
from ai_backend.services.notifications import (
    send_video_notification, send_overcrowd_notification
)
from ai_backend.inference.audio_inference import (
    load_audio_models, predict_cry_from_file,
    ensure_temp_audio_folder
)
from ai_backend.config import (
    TEMP_IMAGE_FOLDER, AUDIO_SAMPLE_RATE,
    AUDIO_CHUNK_DURATION_SECONDS, AUDIO_DEVICE_INDEX, TEMP_AUDIO_FOLDER,
    TEMP_VIDEO_FOLDER, MAX_PERSON_COUNT
)
...
async def lifespan(app: FastAPI):
    ...
    print(f"디바이스 : {device}")
    from ai_backend.inference.fall_detection_pipeline import initialize_models
